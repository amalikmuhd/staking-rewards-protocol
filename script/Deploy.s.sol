...
/// @notice Deploys StakingRewards and RewardNFT, then wires them together.
...
///      The broadcasting account must be OWNER, since setRewardNFT is onlyOwner.
///      Example:
///        forge script script/Deploy.s.sol:DeployStakingRewards \
///          --rpc-url $RPC_URL --broadcast --verify
contract DeployStakingRewards is Script {
    function run() external returns (StakingRewards staking, RewardNFT rewardNFT) {
        ...
        rewardNFT = new RewardNFT(address(staking));
        staking.setRewardNFT(address(rewardNFT));
        vm.stopBroadcast();
